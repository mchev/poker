...
import { Mail, RefreshCw, Trash2, Users } from 'lucide-vue-next';
import { ref } from 'vue';
...
import { casinoChipPrimary, pokerCard, pokerHeader, pokerMuted, pokerPanel } from '@/lib/pokerUi';
    ...
    <Card :class="pokerCard">
        <CardHeader :class="pokerHeader">
            ...
            <CardDescription :class="pokerMuted">
                Actions réservées à l’administrateur ({{ adminParticipants.length }}
                inscrit{{ adminParticipants.length > 1 ? 's' : '' }}).
            </CardDescription>
        </CardHeader>
                ...
                >
                    <div class="min-w-0">
                        <p class="font-medium text-white">
                            {{ listed.name }}
                            <span
                                v-if="listed.id === currentParticipantId"
